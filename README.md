...
This project broadens knowledge of Docker by requiring custom Dockerfiles, a docker-compose.yml for orchestration, and a Makefile for build/run management. It emphasizes best practices like no infinite loops, proper PID 1 handling, and secure credential management.

### Docker Usage and Design Choices
Docker is used to containerize each service (MariaDB, WordPress, Nginx), allowing isolated, reproducible environments. The project sources include custom Dockerfiles (based on Debian Bookworm), scripts for initialization (init-db.sh, setup-wp.sh), and configuration files (nginx.conf, php-fpm pool adjustments). The docker-compose.yml defines services, networks, and volumes for seamless integration.
...
Environment variables are used here via .env for simplicity and dev-friendliness, with .gitignore to avoid commits. Secrets could be added for production (e.g., db_password.txt).
...
| **Security**       | Better (firewall between containers)| Exposed to host vulnerabilities |
...
3. **Set Environment**: Copy `.env.example` to `.env` and fill in values (domain, passwords – never commit!).
4. **Build and Run**: `make all` (or `make up` for quick start).
5. **Access Site**: https://mhummel.42.fr (self-signed cert – proceed anyway). Admin: https://mhummel.42.fr/wp-admin.
6. **Stop**: `make down`.
7. **Clean**: `make clean` (removes images) or `make fclean` (full reset).

## Resources
- **Docker Documentation**: [Official Docker Docs](https://docs.docker.com/) – Used for best practices (PID 1, no infinite loops).
...
- **AI Usage**: Grok (xAI) assisted in debugging Docker scripts (e.g., init-db.sh waiting loops, PHP-FPM paths) and generating comparisons (VMs vs Docker). No code generation – all custom-written by me. Tools used: Code execution for testing bash snippets.
